...
class GPT_XAttn(nn.Module):
    def __init__(self, cfg: Config, is_critic: bool):
        ...
        out_dim = cfg.n_embed if not is_critic else 1
...
class Transformer(nn.Module):
    def __init__(self, cfg: Config, act_limit: Tensor, use_adaln: bool):
        ...
        self.action_mean = nn.Linear(cfg.n_embed, 1)
        self.action_std = nn.Linear(cfg.n_embed, 1)
        self.value_head: Callable[[Tensor], Tensor] = nn.Linear(cfg.action_dim, 1)
    ...
    def get_action_and_value(self, state: Tensor) -> Tensor:
        """
        Get action from the actor network.
        """
        B = state.size(0)
        actions = torch.zeros(B, self.act_dim, device=self.device).unsqueeze(-1)  # (B, act_dim, 1)
        action_out = self.actor(state, actions)
        action_mean = self.action_mean(action_out)
        action_std = self.action_std(action_out)

        value: Tensor = self.critic1(state, actions)
        value = self.value_head(value.transpose(-1, -2)).squeeze(-1)  # (B, 1)
...
if __name__ == "__main__":
    gpt_config = Config()
    tf = Transformer(gpt_config, act_limit=0, use_adaln=False)
    ...
    actions = tf.get_action_and_value(state)
